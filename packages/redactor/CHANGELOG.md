...
- [#16](https://github.com/Studnicky/mnemotek/pull/16) [`6b81aba`](https://github.com/Studnicky/mnemotek/commit/6b81aba1943e982d90768afec6c94a65be2465a7) Thanks [@Studnicky](https://github.com/Studnicky)! - Harden the gain-tracking log (`.redactor/gain.ndjson`). Entries read from disk are now validated against `GainEntryEntity` instead of a bare type assertion, so a truncated or corrupted line (e.g. from a killed process mid-write) is dropped instead of silently trusted. The log is also capped at 1000 entries, trimming the oldest once exceeded, so it no longer grows unbounded.
