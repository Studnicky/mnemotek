...
export function createMnemotekEslintConfig (options) {
  const {
    tsconfigRootDir,
    sourceFiles = ['src/**/*.ts'],
    testFiles = ['tests/**/*.ts'],
    sourceProject = ['./tsconfig.json'],
    testProject = ['./tsconfig.tests.json'],
    ignores = ['dist/**', 'node_modules/**', 'site/**', 'scripts/**', '**/*.d.ts', 'eslint.config.mjs', 'eslint.config.shared.mjs', 'packages/**'],
    extraSourceRules = {},
    extraTestRules = {},
    hexagonal = true
  } = options
  ...
  const configs = [
    ...
    ...tseslint.configs.all,
    {
      ...
      languageOptions: {
        ...
        parserOptions: {
          project: [...sourceProject, ...testProject],
          tsconfigRootDir
        }
      }
    },
    {
      ...
      rules: {
        ...
        // @studnicky/type-alias-invariants mandates interfaces for contract shapes
        // (dynamic maps, single-call-signature types); these two generic rules from
        // tseslint.configs.all demand the opposite for the same shapes. The custom,
        // purpose-built rule wins.
